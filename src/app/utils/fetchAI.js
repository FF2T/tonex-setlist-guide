...
function fetchAI(song, gId, banksAnn, banksPlug, aiProvider, aiKeys, guitars, feedback, availableSources, recoMode, guitarBias, outputContext, preferredStyles, basses, bassAmps, guitarAmps, pedals) {
  ...
  * settings_guitar : si l'artiste empile guitare-volume + pédale, mentionne le réglage volume guitare (ex: volume 8 pour clean, 10 pour saturer la pédale au max).`
  ...
  // guitar_amp_settings : réglages des VRAIS potards 0-10 pour cet ampli, par
  // morceau. Distinct de la capture ToneX (preset_ann/plug). Parallèle à
  // bass_recommendation.amp_settings.
  const hasGuitarAmpContext = Array.isArray(guitarAmps) && guitarAmps.length > 0;
  const guitarAmpContextLine = (() => {
    if (!hasGuitarAmpContext) return '';
    const list = guitarAmps.map((a) => `- ${a.name} (${a.brand}, ${a.wattage}W, channels: ${(a.channels || []).join('/')}, potards: ${(a.knobs || a.eq || []).join('/')})`).join('\n');
    return `\nAMPLIS GUITARE TRADITIONNELS DISPONIBLES :\n${list}\n\nL'utilisateur joue parfois sur un ampli guitare PHYSIQUE (pas seulement la ToneX). RETOURNE un objet "guitar_amp_settings" avec les réglages des VRAIS potards de l'ampli le plus pertinent de cette liste pour CE morceau. Format : {"amp":"<nom EXACT de l'ampli de la liste>","channel":"<canal si pertinent ou ''>","settings":{"<potard exact>":valeur,...},"why":{"fr":"...","en":"...","es":"..."}}. Utilise EXACTEMENT les noms de potards fournis pour l'ampli choisi, valeurs 0-10. C'est DISTINCT de la capture ToneX (preset_ann/preset_plug) — ici ce sont les réglages physiques de l'ampli réel. Si aucun ampli de la liste ne convient, mets guitar_amp_settings à null.`;
  })();
  const hasPedalContext = Array.isArray(pedals) && pedals.length > 0;
  const pedalboardContextLine = (() => {
    if (!hasPedalContext) return '';
...
Pour CHAQUE guitare retournée (PAS seulement la 1ère/idéale — TOUTES les
entrées, y compris les alternatives), ajoute OBLIGATOIREMENT "controls" =
réglages des VRAIS contrôles de CE modèle pour le morceau : "selector"
(position du sélecteur de micro, ex. "Position 4 (manche+intermédiaire)" ;
"" si mono-micro/N.A.) + "knobs" (liste {name, value 0-10} des boutons réels)
+ "why" (1 phrase trilingue). Adapte au layout réel : Stratocaster = sélecteur
5 positions + 1 Volume + 2 Tone ; Les Paul/SG = 3 positions + 2 Volumes + 2 Tone ;
Telecaster = 3 positions + 1 Volume + 1 Tone ; guitare 1 micro = pas de sélecteur.
Idem OBLIGATOIRE pour CHAQUE basse de cot_step2_basses (toutes les entrées, pas
seulement la 1ère). ⚠ Les basses Fender (Jazz Bass, Precision) n'ont PAS de
sélecteur de micro → "selector":"" : l'équilibre des micros se fait via les
volumes. Jazz Bass = 2 Volumes (manche + chevalet, leur dosage = le "mix" des
micros) + 1 Tone ; Precision = 1 Volume + 1 Tone (micro split unique). Ne mets
un "selector" non vide que si le modèle a réellement un commutateur de micro.
L'utilisateur peut sélectionner n'importe quelle guitare/basse du rig dans
l'app → chacune doit avoir ses propres "controls" adaptés à son layout réel.
...
- bass_recommendation.cot_step2_basses[].controls.why (Phase 9.8 — réglages micros par basse)
...
- guitar_amp_settings.amp (string nom EXACT ampli), .channel (string), .settings.* (number 0-10) — potards de l'ampli guitare réel ; .why est TRILINGUE
