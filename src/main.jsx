...
export function App() {
  ...
  useEffect(()=>{
    ...
    const profileHash=Object.entries(profiles||{}).map(([id,p])=>
      ...
      // Phase 14 — TOUS les champs zones/optimiseur persistés (câblés d'un
      // coup même si certains pas encore écrits par l'UI, pour ne jamais
      // oublier de déclencher le push Firestore au fil des sous-phases).
      +":"+JSON.stringify(p.bankZones||{})
      +":"+(p.referenceDeviceId||'')
      +":"+JSON.stringify(p.derivationMode||{})
      +":"+(p.jamStyles||[]).slice().sort().join(',')
      +":"+JSON.stringify(p.jamOverrides||{})
      +":"+JSON.stringify(p.discoveryPins||{})
      +":"+JSON.stringify(p.portableTargets||{})
    ).join('|');
    const syncHash=[
  ...
  />;
  else if(screen==="setlists") screenContent=<SetlistsScreen songDb={songDbWithProfileCache} onSongDb={setSongDb} onAiCacheUpdate={setSongAiCache} setlists={mySetlists} allSetlists={setlists} onSetlists={setSetlists} mySongIds={mySongIds} checked={checked} onChecked={setChecked} onSettings={()=>setScreen("profile")} onNavigate={setScreen} banksAnn={banksAnn} onBanksAnn={setBanksAnn} banksPlug={banksPlug} onBanksPlug={setBanksPlug} banksOne={banksOne} banksOnePlus={banksOnePlus} onBanksOne={setBanksOne} onBanksOnePlus={setBanksOnePlus} aiProvider={aiProvider} aiKeys={aiKeys} allGuitars={allGuitars} allRigsGuitars={allRigsGuitars} guitarBias={effectiveGuitarBias} availableSources={availableSources} activeProfileId={activeProfileId} profiles={profiles} profile={profile} onProfiles={setProfiles} onTmpPatchOverride={onTmpPatchOverride} onLive={(slId)=>{setLiveSetlistId(slId||null);setScreen("live");}} toneNetPresets={toneNetPresets} onToneNetPresets={setToneNetPresets} onSharedUsagesOverrides={(reducer)=>setSharedUsagesOverrides((prevMap)=>{const sh={usagesOverrides:prevMap};const next=reducer(sh);return next?.usagesOverrides||{};})}/>;
